...
class MyListTest {
    ...
    @Test
    @DisplayName("Should throw CapacityExceeded Exception when trying to enter more numbers than the capacity of the list")
    void shouldThrowCapacityExceededException() {
        //given, when
        try {
            myList.add(6);
            myList.add(7);
            myList.add(8);
            Assertions.fail("This method should throw CapacityExceededException");
        } catch (CapacityExceeded e) {
            //then
            String message = e.getMessage();
            boolean rightMessage = message.equals("list capacity exceeded.");
